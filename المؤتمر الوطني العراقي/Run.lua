--[[  │rany ﹏ @EEFFI
│تم تطوير وبرمجة السورس من قبل ارماندو ᐂ .
─────▀██▀─────
┊rany ‿ @EEFFI
┊@EEFFI ‿ @EEFFI
#-------------------------------------------------------------------
...
rany = Token:match("(%d+)")
redis:set(rany..":VERSION",1)
redis:set(rany..":SUDO_ID:",SUDO_USER)
redis:set(rany..":DataCenter:",'German')
redis:set(rany..":UserNameBot:",BOT_User)
redis:set(rany..":NameBot:",BOT_NAME)
redis:hset(rany..'username:'..SUDO_USER,'username','@'..GetUser.result.username:gsub('_',[[\_]]))
redis:set("TH3rany_INSTALL","Yes")
info = {}
...
rany = Token:match("(%d+)")
our_id = tonumber(rany)
ApiToken = "https://api.telegram.org/bot"..Token
Bot_User = redis:get(rany..":UserNameBot:")
SUDO_ID = tonumber(redis:get(rany..":SUDO_ID:"))
SUDO_USER = redis:hgetall(rany..'username:'..SUDO_ID).username
version = redis:get(rany..":VERSION")
DataCenter = redis:get(rany..":DataCenter:")
...
..'¦ BOT__INFO: \27[1;34m'.. Bot_User..'\27[0;36m » ('..rany..')\027[0;32m\n'
...
if msg.type == "pv" and redis:get(rany..':mute_pv:'..msg.chat_id_) then
print('\27[1;31m is_MUTE_BY_FLOOD\27[0m')
return false 
end

if redis:get(rany..'sender:'..msg.sender_user_id_..':flood') then
...
if redis:get(rany..'group:add'..msg.chat_id_) then 
...
msg.Rank = 1
elseif redis:sismember(rany..':SUDO_BOT:',msg.sender_user_id_) then 
msg.TheRankCmd = 'المطور '
msg.TheRank = 'مطور البوت '
msg.Rank = 2
elseif msg.GroupActive and redis:sismember(rany..':KARA_BOT:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المنشىء الاساسي'
msg.TheRank = 'المنشىء الاساسي '
msg.Rank = 3
elseif msg.GroupActive and redis:sismember(rany..':MONSHA_BOT:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المنشىء '
msg.TheRank = 'المنشىء '
msg.Rank = 4
elseif msg.GroupActive and redis:sismember(rany..'owners:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المدير ' 
msg.TheRank = 'مدير البوت ' 
msg.Rank = 5
elseif msg.GroupActive and redis:sismember(rany..'admins:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'الادمن '
msg.TheRank = 'ادمن في البوت '
msg.Rank = 6
elseif msg.GroupActive and redis:sismember(rany..'whitelist:'..msg.chat_id_,msg.sender_user_id_) then 
...
local lock_bots = redis:get(rany..'lock_bots'..msg.chat_id_)
ZISBOT = false
...
if msg.GroupActive and ZISBOT and redis:get(rany..'lock_bots_by_kick'..msg.chat_id_) then
kick_user(msg.sender_user_id_, msg.chat_id_)
end
if msg.content_.members_[0].id_ == our_id and redis:get(rany..':WELCOME_BOT') then
SUDO_USER = redis:hgetall(rany..'username:'..SUDO_ID).username
sendPhoto(msg.chat_id_,msg.id_,redis:get(rany..':WELCOME_BOT'),[[•~مـرحبآ آنآ بوت آسـمـي ]]..redis:get(rany..':NameBot:')..[[•~آختصـآصـي حمـآيه‌‏ آلمـجمـوعآت
...
if msg.GroupActive and redis:get(rany..'mute_tgservice'..msg.chat_id_) then
Del_msg(msg.chat_id_,msg.id_)
return false 
end
...
if msg.Admin then redis:srem(rany..'is_silent_users:'..msg.chat_id_,msg.sender_user_id_) return end
...
if msg.Admin then redis:srem(rany..'banned:'..msg.chat_id_,msg.sender_user_id_) return end
...
if redis:get(rany..'mute_text'..msg.chat_id_) then --قفل الدردشه
...
if ScriptFile and ScriptFile.rany then 
if msg.text and ScriptFile.irany then
for k, rany in pairs(ScriptFile.rany) do
local SearchText = msg.text:match(rany)
if SearchText then
if not CheckFlood(msg) then
return false 
end

local GetMsg = ScriptFile.irany(msg,{msg.text:match(rany)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",rany.." | Plugin is: \27[1;32mScript.lua\27[0m")
...
if ScriptFile.drany then
if ScriptFile.drany(msg) == false then
return false
end
...
if Plug.rany then 
if msg.text and Plug.irany then
for k, rany in pairs(Plug.rany) do
local SearchText = msg.text:match(rany)
if SearchText then
if not CheckFlood(msg) then
return false
...
Mohammad = msg.text
if Mohammad then
if redis:sismember(rany..'CmDlist:'..msg.chat_id_,Mohammad) then
mmdi = redis:hget(rany..'CmD:'..msg.chat_id_,Mohammad)
msg.text = Mohammad:gsub(Mohammad,mmdi)
end
end
local GetMsg = Plug.irany(msg,{msg.text:match(rany)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",rany.." | Plugin is: \27[1;32m"..name.."\27[0m")
sendMsg(msg.chat_id_,msg.id_,GetMsg)
end 
return false
end
...
print("The File "..name.." Not Runing in The Source rany")
end 

end
else
print("The File Script.lua Not Runing in The Source rany")
...
function tdcli_update_callback(data)
	...
	redis:srem(rany..'users',msg.sender_user_id_)
	end
	...
	download_file('https://raw.githubusercontent.com/Rany0/rany/master/inc/Run.lua','./inc/Run.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'40% - |█████       |')
	download_file('https://raw.githubusercontent.com/Rany0/rany/master/inc/locks.lua','./inc/locks.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'60% - |███████     |')
	download_file('https://raw.githubusercontent.com/Rany0/rany/master/inc/Script.lua','./inc/Script.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'80% - |█████████   |')
	download_file('https://raw.githubusercontent.com/Rany0/rany/master/inc/functions.lua','./inc/functions.lua')
	download_file('https://raw.githubusercontent.com/Rany0/rany/master/plugins/zhrfa.lua','./plugins/zhrfa.lua')
	download_file('https://raw.githubusercontent.com/Rany0/rany/master/plugins/games.lua','./plugins/games.lua')
...
msg.text = Mohammad:gsub(Mohammad,mmdi)
	...
	mmd = redis:get(rany..":SUDO_ID:")
redis:hset(rany..'username:'..tonumber(mmd),'username',msg.text)
	...
	redis:set(rany..":SUDO_ID:",msg.text)
...
download(stk,32)
	...
	if redis:get(rany..'group:add'..msg.chat_id_) and (msg.sender_user_id_ == SUDO_ID or redis:sismember(rany..':SUDO_BOT:',msg.sender_user_id_) or redis:sismember(rany..':MONSHA_BOT:'..msg.chat_id_,msg.sender_user_id_) or redis:sismember(rany..'owners:'..msg.chat_id_,msg.sender_user_id_) or redis:sismember(rany..'admins:'..msg.chat_id_,msg.sender_user_id_)) then 
	msg.Admin = true
	end
	local lock_bots = redis:get(rany..'lock_bots'..msg.chat_id_)
	ISBOT = false
	...
	if ZISBOT and redis:get(rany..'lock_bots_by_kick'..msg.chat_id_) then
	kick_user(msg.sender_user_id_, msg.chat_id_)
	end
...
if redis:get(rany..'lock:join:'..msg.chat_id_) then
kick_user(msg.sender_user_id_,msg.chat_id_)
	...
	if redis:get(rany..'group:add'..data.chat_.id_) then
	redis:set(rany..'group:name'..data.chat_.id_,data.chat_.title_)
	end
	...
	if redis:get(rany..'group:add-100'..data.channel_.id_) then
	local linkGroup = (redis:get(rany..'linkGroup-100'..data.channel_.id_) or "")
	local NameGroup = (redis:get(rany..'group:name-100'..data.channel_.id_) or "")
	...
	redis:mset(
	rany..'group:name'..k,IDS.Title,
	rany..'num_msg_rany'..k,5,
	rany..'group:add'..k,true,
	rany..'lock_link'..k,true,
	rany..'lock_spam'..k,true,
	rany..'lock_webpage'..k,true,
	rany..'lock_markdown'..k,true,
	rany..'lock_flood'..k,true,
	rany..'lock_bots'..k,true,
	rany..'mute_forward'..k,true,
	rany..'mute_contact'..k,true,
	rany..'mute_document'..k,true,
	rany..'mute_inline'..k,true,
	rany..'lock_username'..k,true,
	rany..'replay'..k,true
	)
	redis:sadd(rany..'group:ids',k) 

	if IDS.Admins then
	for user,ID in pairs(IDS.Admins) do
	redis:hset(rany..'username:'..ID,'username',user)
	redis:sadd(rany..'admins:'..k,ID)
	end
	end
	if IDS.Creator then
	for user,ID in pairs(IDS.Creator) do
	redis:hset(rany..'username:'..ID,'username',user)
	redis:sadd(rany..':MONSHA_BOT:'..k,ID)
	end
	end
	if IDS.Owner then
	for user,ID in pairs(IDS.Owner) do
	redis:hset(rany..'username:'..ID,'username',user)
	redis:sadd(rany..'owners:'..k,ID)
	end
	end
	end
	io.popen("rm -fr ../.telegram-cli/data/document/*")
	sendMsg(Uploaded_Groups_CH,Uploaded_Groups_MS,'•~** تم رفع آلنسـخه‏‏ آلآحتيآطـيه\n•~** حآليآ عدد مـجمـوعآتك هيه *'..redis:scard(rany..'group:ids')..'* ®️\n✓')
	end
	elseif data.ID == "UpdateUser" then  
	if data.user_.type_.ID == "UserTypeDeleted" then
	print("¦ userTypeDeleted")
	redis:srem(rany..'users',data.user_.id_)
	elseif data.user_.type_.ID == "UserTypeGeneral" then
	local CheckUser = redis:hgetall(rany..'username:'..data.user_.id_).username
	...
	redis:hset(rany..'username:'..data.user_.id_,'username',USERNAME)
	end 
	end
